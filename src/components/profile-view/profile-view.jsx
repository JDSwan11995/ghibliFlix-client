...
export const ProfileView = ({ movies, onLoggedOut }) => {
  ...
  if (storedUser?.FavoriteMovies) {
    storedUser.FavoriteMovies + storedUser.FavoriteMovies.map(String);
  }
  ...
  const handleUpdateFavorites = (updateFavorites) => {
    const updatedUser = {
      ...user,
      FavoriteMovies: updateFavorites,
    };
    setUser(updatedUser);
    localStorage.setItem("user", JSON.stringify(updatedUser));
  };
